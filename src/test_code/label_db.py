...
conn = MongoClient(host='192.168.106.170')
# print(conn)
# print(conn.database_names())

fileDB = conn.get_database('fileDB')
recommend = conn.get_database('recommend')

model_doc = recommend['model_doc']
listRec = fileDB['listRec']

# print(model_doc.count())
# print(model_doc.count({"sentList": { "$exists": True }}))


#####################
# # mongo 分段 分句

# cur = model_doc.find({}).limit(1)

# # cur = model_doc.find({"sentList": { "$exists": False }}, {"docId": True, "_id": False}, cursor_type=CursorType.EXHAUST)
# cur = model_doc.find({}, {"docId": True, "_id": False}, cursor_type=CursorType.EXHAUST)
# # cur = model_doc.find({"sentList": { "$exists": False }}, {"docId": True, "_id": False}).limit(10000)
...
#################
# mongo 清洗非法字符
# cur = model_doc.find({'docId': "e55ad04e-a8d9-4fa9-80d9-b0e263009f3f"}).limit(1)

doc_id_list = []

with codecs.open('./doc_extra.txt') as f:
    doc_id_list = [str.strip(line) for line in f]


# doc_id_list = ['87695de4-88ea-4a6a-957d-38de324fdf7b',
# doc_id_list.append('98024173-35f3-429e-9464-1efc5c9963d1')
# doc_id_list.append('5e0a0bf3-952c-4edd-b5e0-f7bad9ae8630')
# doc_id_list.append('adef5295-44c5-4a0c-bd78-1aba9a19bae0')
# doc_id_list.append('c0d403ad-1fab-428f-a284-e3a222d0b620')
# doc_id_list.append('bf1155d8-084e-4ffd-b24b-815e91a2f618')
# doc_id_list.append('6f5f6575-1326-4a9b-9853-4741dd3e3688')

# cur = model_doc.find({'docId': { '$in': doc_id_list}})
# cur = model_doc.find({"schema_version": "2.0"})
# cur = model_doc.find({}).limit(10)
cur = model_doc.find({}, cursor_type=CursorType.EXHAUST)

w_file = codecs.open('./maybe.txt', 'w', encoding='utf8')

loop_counter = 0
for result in cur:
    # print(dumps(dict(result), ensure_ascii=False, indent=2))

    # print(result['docContent'])

    if loop_counter % 100 == 0:
        print(loop_counter, ':', result['docId'])
    loop_counter += 1
    ...
    # print(doc_id, case_summary_lineno, _line[:40])
    if case_summary_lineno is None:
    ...
    # pprint([line for line in case_summary_sent_list])
    ...
    if '$' in _line:
        ...
        case_summary_piece_lineno, _line = detect_case_summary_lineno(case_summary_piece_list, merge_limit=len(case_summary_piece_list))
    ...
    model_doc.update_one(query_parameter, update_parameter)

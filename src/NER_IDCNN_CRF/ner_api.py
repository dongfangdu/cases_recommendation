...
from .data_utils import iobes_iob, input_from_line
...
class NerApi:
    ...
    with open(os.path.join(root_p,"maps.pkl"), "rb") as f:
        char_to_id, id_to_char, tag_to_id, id_to_tag = pickle.load(f)
    with open(os.path.join(root_p,"config_file"), encoding="utf8") as f:
        config = json.load(f)
    def __init__(self):
    ...
    def get_ner(self,st):
        return self.model.evaluate_line(self.sess, input_from_line(st, self.char_to_id), self.id_to_tag)
if __name__ == "__main__":
    print(NerApi().get_ner("此事交给王亚东去办"))
